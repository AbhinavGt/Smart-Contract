...
def _grounding_section(finding: dict[str, Any]) -> str:
    warning = finding.get("grounding_warning")
    return f"**Grounding warning:** {warning}\n\n" if warning else ""
...
def _render_fix_section(finding: dict[str, Any]) -> str:
    ...
    fallback_note = ""
    if fix.get("is_fallback"):
        fallback_note = (
            " (Note: this fix was generated by a rule-based offline fallback, "
            "not the LLM; gates verify mechanical properties only)"
        )
    if fix.get("gate_failed") == "scope":
        lines = [
            "**Fix verification:**",
        ...
        lines = [
            "**Fix verification:**",
            f"{'✅' if status.lower() == 'verified' and not fix.get('is_fallback') else '⚠️'} "
            f"{status}{fallback_note} — gates passed: {gate_info}",
        ]
    if fix.get("detail"):
    ...
    *,
    ...
    for index, finding in enumerate(findings_with_explanations, start_index):
        confidence = (
            ...
            if finding.get("is_fallback")
            else ("✅ Confident" if finding.get("confident", True) else "⚠️ Uncertain")
        )
        kind = finding.get("type", finding.get("check", "unknown"))
        sections.append(
            f"### Finding {index}: {kind} — Severity: {finding.get('impact', 'Unknown')}\n"
            f"**Explanation confidence:** {confidence} "
            f"(critic loops used: {finding.get('loops_used', 'unknown')})\n"
            f"**Location:** {_location(finding)}\n\n"
            f"{_grounding_section(finding)}"
...
def format_report(
    ...
    for index, finding in enumerate(findings_with_explanations, 1):
        ...
        sections.append(
            f"## Finding {index}: {finding.get('check', 'unknown')} — "
            f"Severity: {finding.get('impact', 'Unknown')}\n"
            f"**Explanation confidence:** {confidence} "
            f"(critic loops used: {finding.get('loops_used', 'unknown')})\n"
            f"**Location:** {_location(finding)}\n\n"
            f"{_grounding_section(finding)}"
